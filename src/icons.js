...
function createDefs() {
	...
	stop1.classList.add('stop1');
	stop1.setAttribute('offset', '0%');
	const stop2 = document.createElementNS(
		'http://www.w3.org/2000/svg',
		'stop'
	);
	stop2.classList.add('stop2');
...
function createPath(d, transform) {
...
function createIcon(data) {
	const SVG = createSVG(data.viewBox);
	const defs = createDefs();
	const paths = [];
	data.paths.forEach((pathData) => {
		const path = createPath(pathData.d, pathData.transform);
		paths.push(path);
	});
...
const clearDayIcon = createIcon(iconData.clearDay);
const clearNightIcon = createIcon(iconData.clearNight);
const cloudyIcon = createIcon(iconData.cloudy);
const foggyIcon = createIcon(iconData.foggy);
const partlyCloudyNightIcon = createIcon(iconData.partlyCloudyNight);
const partlyCloudyDayIcon = createIcon(iconData.partlyCloudyDay);
const rainIcon = createIcon(iconData.rain);
const snowIcon = createIcon(iconData.snow);
const windIcon = createIcon(iconData.wind);
